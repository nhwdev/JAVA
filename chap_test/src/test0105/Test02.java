...
class Buyer{
	...
	void buy(Food f){
		if(money>f.price) {
			...
			money -= f.price;
			point += f.point;
			System.out.println(f+"을(를) "+ f.price + "가격에 구입");
	...
	void summary() {
		...
		for(int i=0; i<cnt; i++) {
			...
			totNames += cart[i].toString()+(i==cnt-1?"":", ");
		...
		System.out.println("총 구매 금액: "+ totPrice);
		System.out.println("총 구매 목록: "+ totNames);
...
public class Test02 {
	public static void main(String ... args) {
		...
		Cidar sidar = new Cidar(1500,1000);//가격, 용량(ml)
